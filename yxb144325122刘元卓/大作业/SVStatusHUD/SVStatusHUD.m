...
{
    [SVStatusHUD showWithImage:[UIImage imageNamed:@"hud_smail.png"] status:status];
}
+ (void)showSuccessWithStatus:(NSString *)status
{
    [SVStatusHUD showWithImage:[UIImage imageNamed:@"hud_success.png"] status:status];
}
+ (void)showErrorWithStatus:(NSString *)status
{
    [SVStatusHUD showWithImage:[UIImage imageNamed:@"hud_error.png"] status:status];
}
+ (void)showWifiWithStatus:(NSString *)status
{
    [SVStatusHUD showWithImage:[UIImage imageNamed:@"hud_wifi.png"] status:status];
}
...
- (void)setStatus:(NSString *)string
{
    ...
    // 中部消息小面板的背景颜色,默认为黑色
    self.hudView.backgroundColor = [UIColor colorWithRed:0.047 green:0.6196 blue:0.9955 alpha:0.7];
    ...
    {
        // 有文字的情况下,图片略微偏上
        self.imageView.center = CGPointMake(self.hudView.bounds.size.width/2, 60);
    }
...
- (UILabel *)stringLabel {
    if (stringLabel == nil) {
        stringLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 90, 160, 20)];
...
- (SVStatusImage *)imageView {
    if (imageView == nil) {
        imageView = [[SVStatusImage alloc] initWithFrame:CGRectMake(0, 0, 28, 28)];
