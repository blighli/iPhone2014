...
@property (nonatomic, strong)NSMutableArray *enroll;

@end

@implementation EnrollTVC

- (void)viewDidLoad {
    [super viewDidLoad];
    
    NetworkTool *networkTool = [[NetworkTool alloc]init];
    [networkTool getPostWithType:@"enroll" success:^(id responseData) {
        NSLog(@"%@", responseData);
        self.enroll = responseData;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return [self.enroll count];
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
    Post *post = [self.enroll objectAtIndex:indexPath.row];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    postDetailViewControl.post = [self.enroll objectAtIndex:index];
}
