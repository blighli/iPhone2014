...
@property (nonatomic, strong)NSMutableArray *job;

@end

@implementation JobTVC

- (void)viewDidLoad {
    [super viewDidLoad];
    
    NetworkTool *networkTool = [[NetworkTool alloc]init];
    [networkTool getPostWithType:@"job" success:^(id responseData) {
        self.job = responseData;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    Post *post = [self.job objectAtIndex:indexPath.row];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    postDetailViewControl.post = [self.job objectAtIndex:index];
}
