...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (editingStyle == UITableViewCellEditingStyleInsert) {
        ...
        XYZAddToDoItemViewController* addToDoItemController = [[XYZAddToDoItemViewController alloc] initWithNibName:@"XYZAddToDoItemViewController" bundle:nil];
        [self performSegueWithIdentifier:@"XYZAddToDoItemViewController" sender:self];
//        [self.navigationController pushViewController: addToDoItemController animated:true];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *message = [[NSString alloc] initWithFormat:
                         @"You selected %ld", (long)indexPath.row];
    UIAlertView *alert = [[UIAlertView alloc]
                          initWithTitle:@"change your task!"
                          message:message
                          delegate:self
    ...
    //tappedItem.itemName = @"hahaha";
    
    //UITextField * content = [alert textFieldAtIndex:0];
    NSLog(@"haha\n");
   
    //tappedItem.itemName = content.text;
...
-(void) alertView : (UIAlertView*)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    XYZToDoItem *tappedItem = [self.toDoItems objectAtIndex:self.selectedRow.row];
