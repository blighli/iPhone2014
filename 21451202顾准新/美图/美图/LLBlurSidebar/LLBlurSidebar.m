...
- (void)viewDidLoad {
    ...
    // 改动
    // 列表
    CGRect rect = CGRectMake(kSidebarWidth*2, 0, kSidebarWidth, self.view.bounds.size.height);
...
- (BOOL)isSidebarShown{
    ...
    return  self.contentView.frame.origin.x < kSidebarWidth*2 ? YES : NO;
}

- (void)showHideSidebar
{
    //change
    if(self.contentView.frame.origin.x == kSidebarWidth*2){
...
- (void)autoShowHideSidebar
{
    ...
    else
    {
        ...
        [UIView animateWithDuration:duration animations:^{
            //[self setSidebarOriginX:-kSidebarWidth];
            //change
            [self setSidebarOriginX:kSidebarWidth*2];
        } completion:^(BOOL finished) {
            _isMoving = NO;
            self.view.hidden = YES;
        }];
...
{
    // 记录按下时的x位置
    //NSLog(@"%f",self.contentView.frame.origin.x);
    startContentOriginX = self.contentView.frame.origin.x;
    
    //NSLog(@"%f",startContentOriginX);
...
//        if (offsetX > 40 || ((int)startContentOriginX==0 && offsetX<0 && offsetX>-40)) // 右滑大于40，或展开时左滑一丁点
        if(offsetX > 40 || ((int)startContentOriginX == kSidebarWidth && offsetX<0 && offsetX >-40))
        {
...
- (void)setSidebarOffset:(CGFloat)offset
{
    ...
    if (offset >=0) { // 右滑
        // 如果不在最右
        if (rect.origin.x <kSidebarWidth*2) {
            rect.origin.x = startContentOriginX + offset;
            if (rect.origin.x < kSidebarWidth*2) {
                rect.origin.x = kSidebarWidth*2;
            }
        }
