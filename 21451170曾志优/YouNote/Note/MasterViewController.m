...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.section==1 ){
       ...
       cell.imageView.image =[UIImage imageWithData:scrawl.binScrawl];
    }
    if( indexPath.section==2){
        Image *myImage = data[indexPath.section][indexPath.row];
        
        cell.imageView.image =[UIImage imageWithData:myImage.binImage];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        if(indexPath.section==0){
            NoteRecord *nRecord =data[indexPath.section][indexPath.row];
            NSLog(@"%d",nRecord.row);
            deleteSQL = [NSString stringWithFormat: @"delete from notes where topic = %d",nRecord.row];
        }else if(indexPath.section==1){
            Scrawl *scrawl =data[indexPath.section][indexPath.row];
            deleteSQL = [NSString stringWithFormat: @"delete from scrawl where topic = %d",scrawl.row];
        }else{
            Image *image =data[indexPath.section][indexPath.row];
             deleteSQL = [NSString stringWithFormat: @"delete from image where topic = %d",image.row];
