...
//字符串模除
+ (NSString *)modV1:(NSString *)v1 v2:(NSString *)v2 {
    CGFloat result = [v1 intValue] % [v2 intValue];
    return [NSString stringWithFormat:@"%g", result];
}
...
+ (NSString *)calcNormalFormula:(NSString *)formula {
    ...
    NSRange modRange = [formula rangeOfString:@"%" options:NSLiteralSearch];
    NSLog(@"here");
    // 只包含加减的运算
    if (mulRange.length == 0 && divRange.length == 0 && modRange.length == 0) {
        return [self calcSimpleFormula:formula];
    }
    // 进行乘除运算
    /*int index = mulRange.length > 0 ? mulRange.location : divRange.location;*/
    int temp =mulRange.location < divRange.location ? mulRange.location : divRange.location;
    int index = modRange.location < temp ? modRange.location : temp;
    NSLog(@"modrange%ld",modRange.location);
    // 计算左边表达式
    ...
    if (index == mulRange.location) {
        ...
        tempResult = [self modV1:num1 v2:num2];
    }
    // 代入计算得到新的公式
