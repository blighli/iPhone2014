...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if( [temp isEqualToString:@"0"]==YES )
    {
        cell.detailTextLabel.text = @"文字";
    }
    else if( [temp isEqualToString:@"1"]==YES )
    {
        cell.detailTextLabel.text = @"照片";
    }
    else
    {
        cell.detailTextLabel.text = @"手写";
    }
    return cell ;
    return nil;
}
...
- (void)viewDidLoad {
    ...
    [self querySql:@"select id,title,type,info from contents" database:@"XiaoBenZi_1087.db"];
