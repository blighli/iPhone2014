...
- (void)viewDidLoad {
    ...
    titleStrs=[[NSArray alloc]initWithObjects:@"早餐",@"午餐",@"晚餐",@"饮料",@"零食",@"交通",@"购物",@"娱乐",@"社交",@"衣物", nil];
...
- (void)receiveData:(NSInteger) flag {
    ...
    NSString *paramstr;
    if (flag==0) {
        url=[NSURL URLWithString:@"http://localhost:8080/expend/countByYear"];
        paramstr=[NSString stringWithFormat:@"username=%@&&year=%ld",username,year];
    } else if(flag==1) {
        url=[NSURL URLWithString:@"http://localhost:8080/expend/countByMonth"];
        paramstr=[NSString stringWithFormat:@"username=%@&&year=%ld&&month=%ld",username,year,month];
    } else {
        url=[NSURL URLWithString:@"http://localhost:8080/expend/countByDay"];
